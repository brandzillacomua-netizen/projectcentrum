...
async function main() {
  const { data: users, error } = await supabase.from('system_users').select('*').ilike('last_name', '%пілецьк%')
  console.log('Roman Piletsky user record:', JSON.stringify(users, null, 2))
}
