...
async function checkAllWarehouses() {
  ...
  const { data: inv } = await prodClient.from('inventory').select('warehouse');
  const counts = {};
  inv?.forEach(i => {
    counts[i.warehouse] = (counts[i.warehouse] || 0) + 1;
  });
  console.log('Warehouse counts in PROD:', counts);
}

checkAllWarehouses().catch(console.error);
