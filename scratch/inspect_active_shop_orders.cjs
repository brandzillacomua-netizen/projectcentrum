...
async function run() {
  ...
  orders.forEach(o => {
    ...
    const oCards = cards.filter(c => c.order_id === o.id);
    const activeCards = oCards.filter(c => c.status !== 'completed');
    console.log(`Order: ${o.order_num} (${o.customer || ''}) | ID: ${o.id} | Status: ${o.status} | Tasks: ${oTasks.length} | ActiveCards: ${activeCards.length}`);
  });
}
