...
async function seed() {
  console.log("Seeding cutter types...");
  for (const cutter of defaultCutters) {
    // Check if a nomenclature with this name already exists
    ...
    if (existing && existing.length > 0) {
      ...
      if (record.type !== 'cutter_type') {
        // Update type to cutter_type
        ...
        if (updErr) {
          ...
          console.log(`Updated existing nomenclature ${cutter.name} to cutter_type.`);
        }
      } else {
        console.log(`Cutter type ${cutter.name} already exists.`);
      }
    } else {
      // Insert new
      ...
      if (insErr) {
        ...
        console.log(`Inserted new cutter type: ${cutter.name}`);
      }
    }
  }
