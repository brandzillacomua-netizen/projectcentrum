...
async function main() {
  ...
  // Delete material_requests for this test task
  await supabase.from('material_requests').delete().eq('task_id', taskId)

  // Delete work_cards for this test task
  await supabase.from('work_cards').delete().eq('task_id', taskId)

  console.log('✅ Cleaned up all test cards and material requests for task:', taskId)
}
