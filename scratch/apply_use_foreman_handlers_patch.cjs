...
const targetCards1 = `      const { error: cardsUpdErr } = await supabase
        .from('work_cards')
...
const targetCutters1 = `      const cutterRequestIds = cutterRequests.map(r => r.id)
...
console.log('SUCCESS: useForemanHandlers.js patched successfully!');
