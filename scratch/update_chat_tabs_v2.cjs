...
// First, safely add a state for newChatType using lines split
...
if (showNewChatIndex !== -1) {
  // Check if it already has newChatType (idempotent)
  if (!lines[showNewChatIndex + 1].includes('newChatType')) {
    lines.splice(showNewChatIndex + 1, 0, "  const [newChatType, setNewChatType] = useState('private')");
  }
} else {
  console.log("Could not find showNewChat state!");
}
