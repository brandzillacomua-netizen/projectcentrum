...
if (navigateIndex !== -1 && !lines.some(l => l.includes('const [searchParams'))) {
  lines.splice(navigateIndex + 1 + spliceOffset, 0, "  const [searchParams, setSearchParams] = useSearchParams()");
  spliceOffset++;
}

if (showNewChatIndex !== -1 && !lines.some(l => l.includes('const [newChatType'))) {
  lines.splice(showNewChatIndex + 1 + spliceOffset, 0, "  const [newChatType, setNewChatType] = useState('private')");
  spliceOffset++;
}

if (activeParticipantsIndex !== -1 && !lines.some(l => l.includes("searchParams.get('support')"))) {
  const supportEffect = [
    "  useEffect(() => {",
...
console.log('Update UI script v4 done');
