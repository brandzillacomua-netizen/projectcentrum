...
async function main() {
  const { data: macs } = await supabase.from('machines').select('id, name, type, sequence_number')
  const names = [...new Set(macs.map(m => m.name))]
  const types = [...new Set(macs.map(m => m.type))]
  console.log('Unique machine names:', names)
  console.log('Unique machine types:', types)
}
