...
# Script to patch SettingsModule.jsx with cutter upload feature
$filePath = "a:\centrum\src\modules\SettingsModule.jsx"
$content = [System.IO.File]::ReadAllText($filePath, [System.Text.Encoding]::UTF8)

# ═══════════════════════════════════════════════════════════════
# PATCH 1: Add cutter state variables after sheetsPreviewList line
# ═══════════════════════════════════════════════════════════════
$oldState = @'
...
# ═══════════════════════════════════════════════════════════════
$oldParseCSV = '  const parseCSV = (text, delimiter = ' + "';')" + @' => {
'@
$newHandlers = @'
  ...
  const processCuttersCSV = (parsedCsv) => {
    ...
    rows.forEach((row, idx) => {
      ...
      if (!name) return
      if (!name.toLowerCase().startsWith('фреза')) return
  ...
  const executeCuttersUpload = async () => {
    ...
    try {
      ...
      for (const item of cuttersPreviewList) {
        ...
        if (!nomRecord) {
          const insertPayload = { name: item.name, type: 'consumable' }
...
$searchParseCSV = "  const parseCSV = (text, delimiter = ';') => {"
$content = $content.Replace($searchParseCSV, $newHandlers + $searchParseCSV)

# ═══════════════════════════════════════════════════════════════
# PATCH 3: Add cutters UI section after SO section closing </section>
# The SO section ends with:
#   </section>
#
#           </div>
#         )}
#       </div>
# We want to insert before "          </div>" which closes the system tab grid
# ═══════════════════════════════════════════════════════════════

# Find the specific closing of the SO section by finding the error block text
$soErrorBlock = @'
              {/* ── ERROR ── */}
...
$cuttersSection = @'
            ...
            <section className="settings-panel glass-panel" style={{ background: '#0e0e11', padding: '30px', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.04)', gridColumn: '1 / -1' }}>
              ...
              <p style={{ fontSize: '0.72rem', color: '#555', marginTop: 0, marginBottom: '24px', lineHeight: '1.5' }}>
                Завантажте CSV-файл залишків фрез зі складу. Файл повинен мати колонки: <strong style={{ color: '#ff9000' }}>«Номенклатура»</strong>, <strong style={{ color: '#60a5fa' }}>«Діаметр ріжучої частини»</strong> та <strong style={{ color: '#10b981' }}>«Залишок на складі»</strong>. Позиції автоматично сортуються за діаметром.
              </p>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  <div style={{
                    ...
                    onDrop={e => {
                      ...
                      if (file) handleCuttersFileChange({ target: { files: [file] } })
                    }}
                  >
                    ...
                    <p style={{ margin: 0, fontSize: '0.7rem', color: '#666', fontWeight: 600 }}>Очікуваний формат: «Номенклатура» | «Діаметр ріжучої частини» | «Залишок на складі»</p>
                  </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.75rem', textAlign: 'left' }}>
                      ...
                      <tbody>
                        {cuttersPreviewList.map((item, i) => {
                          ...
                          const isNewDiamGroup = prevDiam !== item.diameter
                          return (
                            <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)', background: isNewDiamGroup && i > 0 ? 'rgba(255,144,0,0.02)' : 'transparent' }}>
                              <td style={{ padding: '10px 16px', fontWeight: 700, color: '#eee' }}>{item.name}</td>
                              <td style={{ padding: '10px 16px', textAlign: 'center', fontWeight: 900, color: '#60a5fa', fontSize: '0.9rem' }}>
                                {isNewDiamGroup && (
                                  <span style={{ display: 'inline-block', background: 'rgba(96,165,250,0.15)', border: '1px solid rgba(96,165,250,0.3)', borderRadius: '8px', padding: '2px 10px' }}>
                                    Ø {item.diameter}
                                  </span>
                                )}
                              </td>
                        ...
                        {cuttersPreviewList.length === 0 && (
                          <tr><td colSpan={3} style={{ padding: '24px', textAlign: 'center', color: '#555', fontSize: '0.75rem' }}>Жодної фрези не знайдено у файлі</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
              ...
              {cuttersUploadStatus === 'success' && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', padding: '24px 0' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '0.78rem', color: '#aaa', textAlign: 'center' }}>Залишки фрез оновлено. Тепер нач. цеху може обрати фрезу зі складу при формуванні наряду.</p>
                  <pre style={{ background: '#000', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '14px', color: '#00ff66', fontFamily: 'monospace', fontSize: '0.7rem', width: '100%', maxWidth: '640px', maxHeight: '180px', overflowY: 'auto', whiteSpace: 'pre-wrap', margin: 0 }} className="custom-scroll">{cuttersUploadLog}</pre>
...
$content = $content.Replace($soErrorBlock, $cuttersSection)

# Write the patched file back
[System.IO.File]::WriteAllText($filePath, $content, [System.Text.Encoding]::UTF8)

Write-Host "Done! Patched SettingsModule.jsx"
