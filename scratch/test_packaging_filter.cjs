...
async function run() {
  ...
  console.log('=== TEST PACKAGING QUEUE FILTER ===');

  // Filter tasks: exclude tasks that are packaged, exclude deleted/shipped orders
  ...
  console.log('Total Active Batches in Packaging Queue:', Object.keys(batchGroups).length);
  Object.values(batchGroups).forEach(bg => {
    console.log(`Order: ${bg.orderNum} (${bg.customer || ''}) | Status: ${bg.orderStatus} | Tasks count: ${bg.tasks.length}`);
    bg.tasks.forEach(t => console.log(`   Task ${t.id} | Step: ${t.step} | Status: ${t.status}`));
  });
}
