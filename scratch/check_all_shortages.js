...
async function main() {
  ...
  tasks.forEach(task => {
    ...
    Object.keys(snapshot).forEach(nomIdStr => {
      ...
      if (shortage > 0) {
        hasShortage = true
        shortageDetails.push(`Part: "${nom.name}" | Shortage: ${shortage} (Need: ${need}, StockBZ: ${stockBZ}, SheetsMax: ${totalSheetsMax}, Scrap: ${groupScrap})`)
      }
    })

    if (hasShortage) {
      console.log(`Task ID: ${task.id} | Order: "${task.orders?.order_num}" | Step: "${task.step}" has SHORTAGE!`)
      shortageDetails.forEach(d => console.log(`  -> ${d}`))
    }
  })
}
