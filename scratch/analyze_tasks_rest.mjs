...
async function run() {
  console.log("=== ANALYZING TASKS WITH FULL PAGINATION ===")
  ...
  console.log(`Fetched: ${allTasks.length} tasks, ${orders.length} orders, ${workCards.length} work_cards, ${history.length} history records.\n`)
  ...
  for (const task of tasks) {
    ...
    if (EXCLUDED_ORDER_NUMS.includes(orderNum)) {
      console.log(`[EXCLUDED] Order #${orderNum} (Task ID: ${task.id}) is in explicit exclusion list.`)
      continue
    }
    ...
    console.log(`\n--------------------------------------------------`)
    console.log(`[QUALIFIED] Order #${orderNum} | Task ID: ${task.id} | Product: "${productName}"`)
  ...
  for (const q of qualifyingTasks) {
    ...
    for (const p of q.partsNeedingReissue) {
      console.log(`  - ${p.nom.name}: Shortage ${p.shortage} pcs -> ${p.sheetsNeeded} sheets -> ${p.cardsNeeded} REDO cards (${p.capacity} sheets/card on ${p.machineName})`)
    }
  }
}
