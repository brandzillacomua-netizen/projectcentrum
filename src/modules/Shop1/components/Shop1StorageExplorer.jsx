...
export function Shop1StorageExplorer({
  activeExplorerTab,
  ...
  const explorerTabs = [
    ...
    { id: 'sorting', label: 'СОРТУВАННЯ', icon: <Package size={16} />, color: '#8b5cf6' },
  ...
  const handleBulkMoveToScrap = async () => {
  ...
  const handleSingleMoveToScrap = async (item) => {
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '15px' }}>
          {Object.values(filteredItems.reduce((acc, item) => {
            ...
            const activeTabDetails = explorerTabs.find(t => t.id === activeExplorerTab)
            return (
              <div key={item.id} style={{ background: '#111', borderRadius: '18px', padding: '18px', border: '1px solid #1a1a1a' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ fontSize: '1.5rem', fontWeight: 1000, color: activeTabDetails ? activeTabDetails.color : '#fff' }}>{item.total_qty}</div>
                ...
                {item.type === 'scrap' && (
                  <button
                    onClick={() => handleSingleMoveToScrap(item)}
