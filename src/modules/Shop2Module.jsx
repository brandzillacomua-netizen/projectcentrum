...
const Shop2Module = () => {
  ...
  // VKYA Modal State for Shop 2 Module
  ...
  const handleQCScrapOverride = async () => {
    if (!qcModalCard || qcScrapCount <= 0) return
    if (qcScrapCount > qcModalCard.quantity) {
    ...
    try {
      ...
      const newQty = Math.max(0, qcModalCard.quantity - qcScrapCount)
      ...
      promises.push(
        supabase.from('work_card_history').insert([{
          card_id: qcModalCard.id,
          nomenclature_id: qcModalCard.nomenclature_id,
          stage_name: 'Контроль ВКЯ',
          operator_name: op,
          qty_at_start: qcModalCard.quantity,
          qty_completed: newQty,
          scrap_qty: qcScrapCount,
          started_at: new Date().toISOString(),
          completed_at: new Date().toISOString(),
          is_archived_scrap: true,
          shift_name: qcModalCard.shift_name,
          manager_name: qcModalCard.manager_name,
          machine_name: qcModalCard.machine,
      ...
      promises.push(
        supabase.from('work_cards').update(updatePayload).eq('id', qcModalCard.id)
      )

      // 3. Update inventory scrap_ready stock
      const { data: existing } = await supabase.from('inventory')
        .select('*')
        .eq('nomenclature_id', qcModalCard.nomenclature_id)
        .eq('type', 'scrap_ready')
        .limit(1).maybeSingle()
      ...
      if (existing) {
        promises.push(
          supabase.from('inventory').update({
            total_qty: (Number(existing.total_qty) || 0) + Number(qcScrapCount),
            updated_at: new Date().toISOString()
          }).eq('id', existing.id)
        )
      } else {
        const nom = nomenclatures?.find(n => n.id === qcModalCard.nomenclature_id)
        promises.push(
          supabase.from('inventory').insert([{
            name: nom?.name || 'Деталь',
            unit: nom?.unit || 'шт',
            total_qty: Number(qcScrapCount),
            type: 'scrap_ready',
            nomenclature_id: qcModalCard.nomenclature_id
          }])
        )
      ...
      if (typeof refreshTable === 'function') {
        ...
        refreshTable('work_card_history')
      }
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                                      {c.status !== 'completed' && (
                                        <button
                                          ...
                                          style={{ background: '#ef4444', border: 'none', color: '#fff', padding: '4px 10px', borderRadius: '8px', fontSize: '0.65rem', fontWeight: 900, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '4px', boxShadow: '0 2px 8px rgba(239,68,68,0.3)' }}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.95)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 10025, padding: '40px 20px', overflowY: 'auto' }}>
          <div style={{ background: '#111', width: '100%', maxWidth: '460px', borderRadius: '26px', border: '1px solid #ef444440', overflow: 'hidden', boxShadow: '0 20px 60px rgba(239,68,68,0.15)', margin: 'auto 0' }}>
            <div style={{ padding: '20px 22px', background: '#161616', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #ef444420' }}>
              ...
              <button onClick={() => setQcModalCard(null)} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer' }}><X size={22} /></button>
            </div>
            <div style={{ padding: '24px 22px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
              <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 900 }}>
                {nomenclatures?.find(n => String(n.id) === String(qcModalCard.nomenclature_id))?.name || 'Деталь'}
              </h3>

              {/* Інспектор ВКЯ */}
              <div>
                <label style={{ fontSize: '0.65rem', color: '#444', fontWeight: 900, textTransform: 'uppercase', marginBottom: '7px', display: 'block' }}>ПІБ Інспектора ВКЯ (або відповідального)</label>
                <input
                  type="text"
              ...
              {/* Причина браку */}
              <div>
                <label style={{ fontSize: '0.65rem', color: '#444', fontWeight: 900, textTransform: 'uppercase', marginBottom: '7px', display: 'block' }}>Причина браку</label>
                <select
                  value={qcReason}
                  onChange={e => {
              ...
              {qcReason === 'Інше (коментар)' && (
                <div>
                  <label style={{ fontSize: '0.65rem', color: '#444', fontWeight: 900, textTransform: 'uppercase', marginBottom: '7px', display: 'block' }}>Опишіть іншу причину браку</label>
                  <input
                    type="text"
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '14px' }}>
                  ...
                  <input type="number" min={0} max={qcModalCard.quantity} value={qcScrapCount === 0 ? '' : qcScrapCount} placeholder="0"
                    onChange={e => {
                      ...
                      setQcScrapCount(val === '' ? 0 : Math.max(0, Math.min(qcModalCard.quantity, parseInt(val) || 0)))
                    }}
              ...
              <button onClick={handleQCScrapOverride} disabled={isQcProcessing || qcScrapCount <= 0}
                style={{
                  ...
                  opacity: (isQcProcessing || qcScrapCount <= 0) ? 0.5 : 1
                }}>
                {isQcProcessing ? 'ЗБЕРЕЖЕННЯ...' : '⚠️ ПЕРЕДАТИ В КАРАНТИН ВКЯ'}
              </button>
            </div>
          </div>
        </div>
