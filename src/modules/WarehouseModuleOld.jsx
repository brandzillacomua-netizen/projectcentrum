...
const WarehouseModule = () => {
  ...
  const filteredInventory = inventory.filter(i => {
    ...
    return i.type === activeTab && matchesSearch
  })
              ...
              <tbody>
                {filteredInventory.map(item => (
                  <tr key={item.id} style={{ borderBottom: '1px solid #151515' }}>
                    ...
                    <td style={{ padding: '15px', textAlign: 'center', color: '#ff9000', fontWeight: 900 }}>{activeTab === 'bz' ? (item.bz_qty || 0) : (item.total_qty || 0)} <small style={{ color: '#444', fontWeight: 400 }}>{item.unit}</small></td>
                    <td style={{ padding: '15px', textAlign: 'center', color: Number(item.reserved_qty) > 0 ? '#3b82f6' : '#222', fontWeight: 800 }}>{activeTab === 'bz' ? '—' : (item.reserved_qty || 0)}</td>
            ...
            {filteredInventory.map(item => (
              <div key={item.id} style={{ background: '#111', padding: '15px', borderRadius: '16px', border: '1px solid #222', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '20px' }}>
                  <div><div style={{ fontSize: '0.6rem', color: '#555' }}>{activeTab === 'bz' ? 'ЗАЛИШОК БЗ' : 'НАЯВНІСТЬ'}</div><div style={{ fontSize: '1.2rem', fontWeight: 900, color: '#ff9000' }}>{activeTab === 'bz' ? (item.bz_qty || 0) : (item.total_qty || 0)}</div></div>
                  {activeTab !== 'bz' && <div><div style={{ fontSize: '0.6rem', color: '#555' }}>РЕЗЕРВ</div><div style={{ fontSize: '1.2rem', fontWeight: 900, color: '#3b82f6' }}>{item.reserved_qty || 0}</div></div>}
                </div>
              </div>
            ))}
          </div>
