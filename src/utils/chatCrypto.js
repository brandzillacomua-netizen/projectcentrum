// ─── CHAT CLIENT-SIDE ENCRYPTION (ChaCha20 Stream Cipher) ────────────────────
// Pure JavaScript implementation of ChaCha20 cipher (RFC 7539).
// Guaranteed to work on ALL browsers and network origins (HTTP, HTTPS, local IP).
...
// Derive a 256-bit key (8 u32 words) from seed string using FNV-1a cascade
function deriveThreadKeyWords(str) {
...
/**
 * Synchronous / Async Encryption function (ChaCha20)
 */
export const encryptChatMessage = (text, threadId) => {
  if (!text || typeof text !== 'string' || !text.trim()) return text
  if (text.startsWith('[ENC:v1:')) return text

  try {
    const keyWords = deriveThreadKeyWords(`${MASTER_SALT}:${threadId || 'global'}`)
...
/**
 * Synchronous / Async Decryption function (ChaCha20)
 */
export const decryptChatMessage = (text, threadId) => {
...
/**
 * Decrypts an array of message objects synchronously/async.
 */
export const decryptMessageList = (messages = [], threadId) => {
  if (!Array.isArray(messages) || messages.length === 0) return messages
  return messages.map((msg) => {
    if (!msg) return msg
    if (msg.body && typeof msg.body === 'string' && msg.body.startsWith('[ENC:v1:')) {
      const decryptedBody = decryptChatMessage(msg.body, msg.thread_id || threadId)
